...
function stringToNumber(str) {
  ...
  const string = Number(str);  // Використовуємо Number() для перетворення рядка в число
  return string;  // Повертаємо отримане число
}
...
function isAlmostSame(num1, num2) {
  ...
  return num1 - num2 < Number.EPSILON;  // Визначаємо, чи є різниця між двома числами меншою або рівною EPSILON
}
...
function isUnsafeInteger(num) {
  ...
  return !Number.isSafeInteger(num); // Використовуємо логічну оператор НЕ (!), щоб отримати протилежну відповідь від функції isSafeInteger()
}
...
function isAlmostZero(num) {
  return num > Number.MIN_VALUE; // Перевіряємо, чи є число більше 0, але все ще менше за Number.MIN_VALUE
}
